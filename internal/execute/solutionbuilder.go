...
func (s *solutionBuilder) buildProject(config string, path tspath.Path, task *buildTask, upStreamStatus []*upToDateStatus) (*upToDateStatus, *taskReporter) {
	...
	projectStatusReport := s.reportProjectStatus(config, status)
	...
	oldProgram := incremental.ReadBuildInfoProgram(task.resolved, incremental.NewBuildInfoReader(s.host))
	...
	taskReporter.program = incremental.NewProgram(program, oldProgram, s.opts.testing)
	compileTimes.changesComputeTime = s.opts.sys.Now().Sub(changesComputeStart)

	taskReporter.exitStatus, taskReporter.statistics = emitAndReportStatistics(
		s.opts.sys,
	...
	// !!! sheetal And update timestamps of files not emitted

	// !!! sheetal oldest output
	if taskReporter.exitStatus == ExitStatusDiagnosticsPresent_OutputsSkipped || taskReporter.exitStatus == ExitStatusDiagnosticsPresent_OutputsGenerated {
...
func (s *solutionBuilder) getUpToDateStatus(config string, path tspath.Path, task *buildTask, upStreamStatus []*upToDateStatus) *upToDateStatus {
	// Config file not found
	...
	buildInfo := s.host.ReadBuildInfo(buildInfoPath)
	if buildInfo == nil {
	...
	oldestOutputFileAndTime := fileAndTime{
		file: buildInfoPath,
		time: s.host.GetModifiedTime(buildInfoPath),
	}
	var newestInputFileAndTime fileAndTime
	// const seenRoots = new Set<Path>();
	for _, inputFile := range task.resolved.FileNames() {
		inputTime := s.host.GetModifiedTime(inputFile)
		if inputTime.IsZero() {
		...
		if inputTime.After(oldestOutputFileAndTime.time) {
			// !!! sheetal
			if buildInfo.IsIncremental() {
				// Check if the input file text has really changed
				// Get resolved input file path
				// Get version
				// Read text and see if version changed
		...
		if inputTime.After(newestInputFileAndTime.time) {
			newestInputFileAndTime = fileAndTime{
				file: inputFile,
				time: inputTime,
			}
		}
	...
	if !task.resolved.CompilerOptions().IsIncremental() {
		...
		for outputFile := range task.resolved.GetOutputFileNames() {
			outputTime := s.host.GetModifiedTime(outputFile)
	...
	for index, upstreamStatus := range upStreamStatus {
		...
		//         return {
		//             type: upToDateStatusTypeInputFileNewer,
		//             outOfDateOutputFileName: buildInfoPath,
	...
	return &upToDateStatus{
		...
		data: &inputOutputFileAndTime{
			input:     newestInputFileAndTime,
			output:    oldestOutputFileAndTime,
...
func (s *solutionBuilder) reportProjectStatus(config string, status *upToDateStatus) *ast.Diagnostic {
	if !s.opts.command.BuildOptions.Verbose.IsTrue() {
		return nil
	}
	switch status.kind {
