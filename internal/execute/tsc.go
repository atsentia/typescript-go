...
func performIncrementalCompilation(
	...
	status, _ := emitAndReportStatistics(
		sys,
		incrementalProgram,
...
func emitAndReportStatistics(
	...
	ioWriter io.Writer,
	compileTimes compileTimes,
) (ExitStatus, *statistics) {
	var statistics *statistics
	result := emitFilesAndReportErrors(sys, programLike, reportDiagnostic, reportErrorSummary, ioWriter, compileTimes)
	...
	if config.CompilerOptions().Diagnostics.IsTrue() || config.CompilerOptions().ExtendedDiagnostics.IsTrue() {
		...
		statistics.report(ioWriter)
	}

	if result.emitResult.EmitSkipped && len(result.diagnostics) > 0 {
		return ExitStatusDiagnosticsPresent_OutputsSkipped, statistics
	} else if len(result.diagnostics) > 0 {
		return ExitStatusDiagnosticsPresent_OutputsGenerated, statistics
	}
	return ExitStatusSuccess, statistics
}
